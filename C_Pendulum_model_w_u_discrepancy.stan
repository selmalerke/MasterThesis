functions {
  ...
  matrix K_pendulum(matrix t_theta, 
    ...
    for (i in 1:(n_theta-1)){
      ...
      for (j in (i+1):n_E){
        K_d[i,j] = exp(-0.5e0 * pow(t_theta[i,1] - t_theta[j,1], 0.2e1) * pow(l_delta, -0.2e1));
        K_d[i,j] = pow(sigma_delta, 0.2e1) * K_d[i,j];
        K_d[j,i] = K_d[i,j];
      }
      K_d[n_theta,n_theta] = pow(sigma_delta, 0.2e1);
    }
    
    K[1:n_theta, 1:n_theta]= K[1:n_theta, 1:n_theta]+K_d[1:n_theta,1:n_theta];
